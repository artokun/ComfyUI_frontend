...
export function useGLSLPreview(
  ...
  let engine: GLSLPreviewEngine | null = null
  ...
  function loadInputImages(): void {
    ...
    for (let slot = 0; slot < node.inputs.length; slot++) {
      ...
      if (imgs?.length) {
        engine.bindInputImage(imageSlotIndex, imgs[0])
      }
      imageSlotIndex++
    }
  }
  ...
  async function renderPreview(): Promise<void> {
    ...
    if (!engine) {
      ...
      try {
        engine = new GLSLPreviewEngine(w, h)
      } catch {
    ...
    const result = engine.compileFragment(source)
    if (!result.success) {
    ...
    for (let i = 0; i < floatValues.value.length; i++) {
      engine.setFloatUniform(i, floatValues.value[i])
    }
    for (let i = 0; i < intValues.value.length; i++) {
      engine.setIntUniform(i, intValues.value[i])
    }
